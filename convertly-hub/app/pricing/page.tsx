...
function planFeatures(plan: PlanDefinition) {
  return [
    ...
    plan.retentionDays === null
      ...
      : `${plan.retentionDays}-day result retention`,
...
function PricingContent() {
  ...
  const requestedPlan =
    status === 'authenticated' && isSubscriptionPlan(checkoutPlan)
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl sm:text-center">
            <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
              Plans for file conversion
            </h1>
            <p className="mt-6 text-lg leading-8 text-gray-600">
              Every plan starts with an account, private history, and transparent file retention.
            </p>
          </div>
          <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-stretch gap-6 lg:max-w-none lg:grid-cols-4">
            {PUBLIC_PLANS.map((plan) => (
              <section
                key={plan.id}
                className={`flex h-full flex-col rounded-lg p-8 ring-1 ring-gray-200 ${plan.isPopular ? 'relative bg-white shadow-lg' : 'bg-white'}`}
      ...
      <PaymentModal
        isOpen={selectedPlan !== null || requestedPlan !== null}
        onClose={closeModal}
        plan={selectedPlan ?? requestedPlan}
      />
    </>
  );
}
...
export default function PricingPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-96 items-center justify-center px-4 py-12 sm:px-6 sm:py-16">
          Loading plans…
        </div>
      }
    >
