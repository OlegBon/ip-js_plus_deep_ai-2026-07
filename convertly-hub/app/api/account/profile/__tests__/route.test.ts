...
describe("account profile", () => { beforeEach(() => { jest.clearAllMocks(); session.mockResolvedValue({ user: { id: "u1" } } as never); });
  ...
  it("returns only safe profile fields", async () => { db.user.findUnique.mockResolvedValue({ name: "Ada", email: "ada@example.com", emailVerified: null, telegramId: null, telegramVerified: null, status: "ACTIVE" } as never); const response = await GET(); await expect(response.json()).resolves.toEqual({ name: "Ada", email: "ada@example.com", emailVerified: false, telegramId: null, telegramVerified: false }); });
