...
describe('account profile', () => {
  ...
  it('does not expose profile without a session', async () => {
  ...
  it('returns only safe profile fields', async () => {
    db.user.findUnique.mockResolvedValue({
      ...
      telegramVerificationTokenHash: 'hash',
  ...
  it('validates and updates only the display name', async () => {
    ...
    const response = await PATCH(
      new Request('http://localhost', { method: 'PATCH', body: JSON.stringify({ name: ' Ada ' }) }),
    );
