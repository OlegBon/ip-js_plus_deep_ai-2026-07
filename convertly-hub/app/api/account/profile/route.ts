...
export async function GET() {
  ...
  const profile = await prisma.user.findUnique({
    where: { id: userId },
    select: { name: true, email: true, emailVerified: true, telegramId: true, telegramVerified: true, status: true },
  });
...
export async function PATCH(request: Request) {
  ...
  const updated = await prisma.user.updateMany({ where: { id: userId, status: "ACTIVE" }, data: { name: name.trim() } });
  if (updated.count === 0) return unauthorized();
  return GET();
}

async function getSessionUserId() { return (await getCurrentSession())?.user?.id; }
