...
import { StoredConversionNotFoundError, downloadStoredConversion } from "@/lib/privacy/conversion-results";
...
describe("GET /api/account/conversions/:conversionId/download", () => {
  ...
  it("streams only the current user's stored result", async () => {
    mockedDownloadStoredConversion.mockResolvedValue({
      body: new ReadableStream({ start(controller) { controller.enqueue(new Uint8Array([112, 110, 103])); controller.close(); } }),
      fileName: "image.png",
      mimeType: "image/png",
    });
