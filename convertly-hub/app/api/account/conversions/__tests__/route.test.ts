...
function formRequest() {
  ...
  return new Request("http://localhost/api/account/conversions", { method: "POST", body: formData });
}
...
describe("POST /api/account/conversions", () => {
  beforeEach(() => {
    ...
    mockedGetSessionPrincipal.mockResolvedValue({ userId: "user-1", storeConversions: true, plan: "FREE" });
    mockedValidateConversionRequest.mockReturnValue({ targetFormat: "png" });
    mockedCreateConversionRequest.mockResolvedValue({
      conversion: { id: "conversion-1", status: "PENDING", createdAt: new Date("2026-08-25T12:00:00.000Z") },
    });
  });
  ...
  it("queues a stored conversion without exposing an API key", async () => {
    ...
    expect(mockedCreateConversionRequest).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ targetFormat: "png" }),
    );
  ...
  it("streams the result when the user has disabled result storage", async () => {
    mockedGetSessionPrincipal.mockResolvedValue({ userId: "user-1", storeConversions: false, plan: "BASIC" });
