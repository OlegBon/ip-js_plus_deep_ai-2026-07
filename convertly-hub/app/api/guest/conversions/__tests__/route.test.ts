...
describe("GET /api/guest/conversions", () => {
  ...
  it("returns a full guest allowance when the visitor has no cookie", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ remainingImage: 3, remainingDocument: 2 });
  ...
  it("hydrates the remaining allowance from the current visitor quota", async () => {
    ...
    mockedPrisma.guestConversionQuota.findUnique.mockResolvedValue({ imageCount: 2, documentCount: 1 } as never);

    const response = await GET();

    await expect(response.json()).resolves.toEqual({ remainingImage: 1, remainingDocument: 1 });
