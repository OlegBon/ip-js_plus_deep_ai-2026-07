...
import { SUPPORTED_SOURCE_MIME_TYPES, SUPPORTED_TARGET_FORMATS } from '@/lib/files/upload-policy';
...
): Promise<ConversionPrincipal | null> {
...
export async function getSessionConversionPrincipal(
  userId: string,
): Promise<ConversionPrincipal | null> {
  ...
  return {
    userId: user.id,
    plan,
    storeConversions: effectiveStoreConversions(plan, user.storeConversions),
  };
}
...
export async function createConversionRequest(
  ...
  const result = await prisma.$transaction(async (transaction) => {
    ...
    if (input.reuseStoredResult && input.sourceFileHash) {
      const existingConversion = await transaction.conversionLog.findFirst({
        where: {
  ...
  if ('existingConversion' in result) return result;
...
export function validateConversionRequest(
  input: CreateConversionInput,
  maxUploadSizeBytes = getPlanDefinition('FREE').maxFileSizeBytes,
): ConversionRequestValidation {
  ...
  if (
    !input.file.name ||
    input.file.size === 0 ||
    !SUPPORTED_SOURCE_MIME_TYPES.has(input.file.type)
  ) {
