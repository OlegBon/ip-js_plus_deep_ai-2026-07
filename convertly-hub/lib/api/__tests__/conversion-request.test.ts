...
describe("conversion request service", () => {
  ...
  it("validates target format and source file before creating a database record", async () => {
    ...
    await expect(
      createConversionRequest(principal, { file: makeFile("archive.zip", "application/zip"), targetFormat: "pdf" }),
    ).resolves.toEqual({ error: "UNSUPPORTED_FILE" });
    await expect(
      createConversionRequest(principal, { file: makeFile("large.png", "image/png", 10 * 1024 * 1024 + 1), targetFormat: "pdf" }),
  ...
  it("creates a pending conversion and updates API-key use time atomically", async () => {
    ...
    mockedPrisma.$transaction.mockImplementation(async (callback) => callback(transaction as never));
