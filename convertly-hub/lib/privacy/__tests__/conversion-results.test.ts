...
describe("conversion result privacy service", () => {
  ...
  it("stores result under a user-scoped private S3 key", async () => {
    ...
    expect(storageKey).toBe("users/user-1/conversions/conversion-1/report%20final.pdf");
  ...
  it("returns a private stored result as a web stream", async () => {
    ...
    const result = await downloadStoredConversion({ conversionId: "conversion-1", userId: "user-1" });
  ...
  it("sanitizes a result name when creating a storage key", () => {
    expect(
      createStoredConversionKey({
        conversionId: "conversion-1",
        userId: "user-1",
    ...
    ).toBe("users/user-1/conversions/conversion-1/.._report.pdf");
  });
});
