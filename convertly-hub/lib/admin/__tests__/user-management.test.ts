...
describe("admin user management service", () => {
  ...
  it("returns a cursor-paginated, secret-free user page", async () => {
    ...
    const result = await listAdminUsers("admin-1", { limit: 2, query: "example", cursor: undefined });

    expect(result).toEqual({
      users: [{ id: "user-3", email: "third@example.com" }, { id: "user-2", email: "second@example.com" }],
      nextCursor: "user-2",
    });
  ...
  it("does not let an administrator suspend their own account", async () => {
    await expect(updateAdminUserStatus("admin-1", "admin-1", "SUSPENDED")).resolves.toBe("SELF_UPDATE_FORBIDDEN");
  ...
  it("updates a user status and revokes any active key atomically", async () => {
    ...
    expect(mockedPrisma.user.updateMany).toHaveBeenCalledWith({ where: { id: "user-1" }, data: { status: "SUSPENDED" } });
  ...
  it("normalizes list bounds and search input", () => {
    ...
    expect(parseAdminUserSearch(params)).toEqual({ limit: 50, query: "a".repeat(100), cursor: undefined });
  });
});
