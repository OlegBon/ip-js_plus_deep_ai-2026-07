...
export default function ApiKeyManager() {
  ...
  useEffect(() => { if (typeof fetch !== "function") { setIsAllowed(false); return; } const controller = new AbortController(); Promise.all([fetch("/api/account/billing", { signal: controller.signal }), fetch("/api/account/api-keys", { signal: controller.signal })]).then(async ([billing, keys]) => { const billingData = billing.ok ? await billing.json() as { activePlan: string } : null; setIsAllowed(billingData?.activePlan !== "FREE"); if (keys.ok) { const data = await keys.json() as { apiKeys: ApiKey[] }; setApiKeys(data.apiKeys); } }).catch(() => setIsAllowed(false)); return () => controller.abort(); }, []);
  async function handleCreate() { const response = await fetch("/api/account/api-keys", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "Dashboard key" }) }); const data = await response.json() as { apiKey?: ApiKey; secret?: string; error?: string }; if (!response.ok || !data.apiKey || !data.secret) { toast.error(data.error ?? "Unable to create API key."); return; } setApiKeys((keys) => [data.apiKey!, ...keys]); setNewSecret(data.secret); toast.success("API key created. Copy it now; it will not be shown again."); }
  async function handleRevoke(id: string) { const response = await fetch(`/api/account/api-keys/${encodeURIComponent(id)}`, { method: "DELETE" }); if (!response.ok) { toast.error("Unable to revoke API key."); return; } setApiKeys((keys) => keys.filter((key) => key.id !== id)); toast.success("API key revoked."); }
  async function copySecret() { if (!newSecret) return; await navigator.clipboard?.writeText(newSecret); toast.success("API key copied to clipboard."); }
