...
export default function UserProfile() {
  ...
  async function handleSaveName() {
    ...
    setProfile(result); setName(result.name ?? ""); setIsEditing(false); toast.success("Profile updated.");
  }
  ...
  function handleEmailVerification() {
    startVerificationTransition(async () => {
      ...
      if (!response.ok) { toast.error(payload.error ?? "Unable to send the verification email."); return; }
  ...
  return <div className="space-y-6 rounded-lg bg-white p-6 shadow-md">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><p className="text-lg font-semibold">{profile.name || "Unnamed user"}</p><p className="text-gray-600">{profile.email}</p><p className="mt-1 text-xs text-gray-500">{profile.emailVerified ? "Email verified." : "Verify your email to keep account recovery available."}</p></div><Button variant="secondary" onClick={() => setIsEditing((value) => !value)}>{isEditing ? "Cancel" : "Edit name"}</Button></div>
    ...
    <div className="border-t border-gray-200 pt-6"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><p className="text-lg font-semibold">Email</p><p className="text-sm text-gray-500">{profile.emailVerified ? "Connected and verified." : "We will send a one-time verification link."}</p></div>{profile.emailVerified ? <span className="text-sm font-medium text-green-700">Verified</span> : <Button onClick={handleEmailVerification} disabled={isVerificationPending} className="w-full whitespace-nowrap md:w-auto">{isVerificationPending ? "Sending…" : "Verify email"}</Button>}</div></div>
