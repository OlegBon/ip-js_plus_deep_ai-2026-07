...
> **Статус на 24 августа 2026.** В репозитории реализованы и протестированы frontend-прототип, Prisma-схема, `GET /api/health`, серверный S3-сервис, NextAuth.js, парольная регистрация/вход, RBAC, Telegram linking и приём запросов на конвертацию через `POST /api/v1/convert`. Маршрут проверяет Bearer API-ключ, валидирует `multipart/form-data` и создаёт `PENDING`-задачу; обработка и выдача результата остаются этапом Core. Dashboard доступен `USER` и `ADMIN`, а `/management` защищён серверной проверкой `ADMIN`. UI данных кабинета пока использует mock-данные.
...
- **Backend ↔ База данных:** `GET /api/health` использует Prisma и PostgreSQL для проверки доступности. `POST /api/v1/convert` находит активный неотозванный API-ключ по SHA-256-хешу, создаёт `ConversionLog` со статусом `PENDING` и обновляет `lastUsedAt` ключа в одной транзакции.
...
- **Backend ↔ Ядро (запланировано):**
  - Легкие задачи: `JPG ↔ PNG` выполняются библиотекой `sharp`.
  - Тяжелые задачи: `DOCX → PDF` делегируются воркеру **Gotenberg**, работающему в отдельном Docker-контейнере.
  - `PDF → DOCX` потребует отдельного best-effort-конвертера: PDF не содержит полной исходной структуры DOCX, поэтому это направление не реализуется через Gotenberg и должно иметь контроль качества результата.
