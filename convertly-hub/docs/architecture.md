...
> **Статус на 22 августа 2026.** Документ описывает целевую архитектуру. В репозитории реализован и протестирован frontend-прототип, Prisma-схема и `GET /api/health`; маршруты аутентификации, конвертации, S3, API-ключей и администрирования ещё не реализованы. Поэтому UI кабинета и панели администратора сейчас использует mock-данные, а маршруты `(dashboard)` не защищены сессией.
...
На 22 августа 2026 локальный Compose-стек проверен запуском: PostgreSQL, MinIO и Gotenberg доступны на опубликованных портах. Это инфраструктурный контур; пользовательские потоки конвертации и S3-хранение пока не реализованы.
...
│   ├── 📂 (dashboard)/      # Группа маршрутов личного кабинета (защита сессией запланирована)
│   │   ├── 📂 dashboard/
│   │   │   └── page.tsx     # Страница личного кабинета
│   │   ├── 📂 management/
│   │   │   └── page.tsx     # Страница панели администратора
│   │   └── layout.tsx       # Макет для личного кабинета
│   ├── 📂 api/health/       # Единственный реализованный API-маршрут
...
│
├── 📂 core/                 # Зарезервировано для логики ядра обработки файлов
│
...
- **Frontend ↔ Backend (целевое состояние):** клиент будет обращаться к Server Actions и API-маршрутам в `/app/api`; сессии будет вести **NextAuth.js**. Сейчас формы реализуют клиентскую демонстрационную логику.

- **Backend ↔ База данных:** `GET /api/health` использует Prisma и PostgreSQL для проверки доступности. Остальные CRUD-маршруты запланированы.

- **Модель данных (миграции применены локально):** Prisma-схема и миграции описывают пользователей, API-ключи и журналы конвертаций, включая роли, статус блокировки, тариф, настройку хранения, S3-метаданные и жизненный цикл ключа. Серверные маршруты, использующие эти модели, ещё не реализованы.

- **Backend ↔ Хранилище S3 (запланировано):** для файлов будет использоваться S3-совместимое хранилище: MinIO локально и облачный провайдер в production.

- **Backend ↔ Ядро (запланировано):**
...
## 6. Потоки аутентификации (целевое состояние)
