...
> **Статус на 24 августа 2026.** В репозитории реализованы и протестированы frontend-прототип, Prisma-схема, `GET /api/health`, серверный S3-сервис и базовый контур сессий NextAuth.js. Маршруты `(dashboard)` защищены серверной проверкой JWT-сессии. Проверка пароля, регистрация, конвертация, API-ключи и серверное администрирование остаются следующими задачами; UI данных кабинета пока использует mock-данные.
...
│   ├── 📂 api/
│   │   ├── 📂 auth/[...nextauth]/ # Обработчики NextAuth.js
...
- **Frontend ↔ Backend:** сессии ведёт **NextAuth.js** через JWT в HttpOnly cookie. `AuthSessionProvider` предоставляет клиенту статус сессии, а серверный layout `(dashboard)` перенаправляет неавторизованных пользователей на `/login`. Проверка учётных данных и подключение формы входа будут добавлены отдельной задачей.
