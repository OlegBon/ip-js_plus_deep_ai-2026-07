...
> **Статус на 25 августа 2026.** В репозитории реализованы и протестированы Prisma-схема, `GET /api/health`, серверный S3-сервис, NextAuth.js, парольная регистрация/вход, восстановление пароля и email-подтверждение через SMTP/MailHog, RBAC, Telegram linking, Core-конвертация, приватность результатов, жизненный цикл API-ключей, тарифные квоты и Mock Checkout, а также server-side API админ-панели. `JPG ↔ PNG` обрабатывается через `sharp`, `DOCX → PDF` — через Gotenberg. Главная страница доступна для ограниченной гостевой конвертации и для browser-конвертации после входа: `POST /api/guest/conversions` не сохраняет файлы и использует месячную cookie-квоту, а `POST /api/account/conversions` использует HttpOnly-сессию, не принимает API-ключ и скачивает готовый результат через session-защищённый маршрут. При включённом хранении результат также помещается в приватный S3; при выключенном — возвращается бинарным потоком без записи в S3. Dashboard доступен `USER` и `ADMIN`, а `/management` защищён серверной проверкой `ADMIN`.
...
| `POST /api/guest/conversions` | Не требуется | Потоковая гостевая конвертация `JPG ↔ PNG`/`DOCX → PDF` до 1 МБ; квота ведётся по HttpOnly cookie и месяцу, S3/история не используются. | `200` — файл; `413`, `415`, `422` — валидация; `429` — квота или короткий IP limiter. |
...
| `POST /api/account/conversions` | Активная сессия NextAuth.js | Принимает browser-загрузку `multipart/form-data` с `file` и `targetFormat`. Повторно использует валидацию, Core, тарифные квоты и политику приватности, не использует API-ключ. | `202` — сохранённая задача принята; `200` — поток результата при выключенном хранении; `401`, `413`, `415`, `422`, `429` — безопасная ошибка доступа, валидации или лимита. |
...
| `GET /api/admin/users` | Активная сессия `ADMIN` | Ищет пользователей по `query` в имени/email с cursor-pagination (`limit` 1–50, `cursor`). | `200` — страница и `nextCursor`; `401` — нет прав. |
