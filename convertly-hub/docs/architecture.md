...
> **Статус на 24 августа 2026.** В репозитории реализованы и протестированы frontend-прототип, Prisma-схема, `GET /api/health`, серверный S3-сервис, NextAuth.js, парольная регистрация/вход, RBAC, Telegram linking, Core-конвертация и управление приватностью результатов. `POST /api/v1/convert` обрабатывает `JPG ↔ PNG` через `sharp` и `DOCX → PDF` через Gotenberg. При включённом хранении результат помещается в приватный S3 и выдаётся только через авторизованный маршрут; при выключенном — возвращается бинарным потоком без записи в S3. Dashboard доступен `USER` и `ADMIN`, а `/management` защищён серверной проверкой `ADMIN`. UI данных кабинета пока использует mock-данные.
