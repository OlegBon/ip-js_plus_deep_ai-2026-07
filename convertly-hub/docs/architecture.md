...
- **Telegram linking и recovery:** авторизованный пользователь получает одноразовый deep link через `POST /api/account/telegram/link`. В базе хранится SHA-256-хеш link-токена и его срок жизни; новая pending-ссылка не отменяет уже подтверждённую привязку. Webhook проверяет секретный HTTP-заголовок и принимает `/start link_<token>` только из private chat, затем однократно привязывает chat ID и нормализованный public username. `DELETE /api/account/telegram/link` owner-scoped очищает Telegram и незавершённый link token. `POST /api/auth/password-reset/request` принимает email или `@username`, всегда отвечает нейтрально и отправляет reset URL Bot API только в уже подтверждённый активный chat. Username не является доказательством владения.
