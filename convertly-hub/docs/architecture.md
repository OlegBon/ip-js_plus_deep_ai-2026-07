...
| `GET`, `POST /api/guest/conversions`                  | Не требуется                                | `GET` возвращает фактический остаток гостевой месячной квоты по HttpOnly cookie; `POST` выполняет потоковую `JPG ↔ PNG`/`DOCX → PDF` до 1 МБ. S3 и история не используются.                                                                                                                                                                                                                                         | `GET 200` — `{ remainingImage, remainingDocument }`; `POST 200` — файл; `413`, `415`, `422` — валидация; `429` — квота или короткий IP limiter.                                                                                                        |
...
| `POST /api/account/conversions`                       | Активная сессия NextAuth.js                 | Принимает browser-загрузку `multipart/form-data` с `file` и `targetFormat`. Перед созданием новой задачи сравнивает SHA-256 исходного содержимого и target format с неистёкшими приватно сохранёнными результатами этого же пользователя; совпадение не расходует квоту и предлагает скачать файл из Dashboard. Повторно использует валидацию, Core, тарифные квоты и политику приватности, не использует API-ключ. | `202` — сохранённая задача принята; `200` — поток результата при выключенном хранении либо `{ status: "AVAILABLE", conversionId }` для уже доступного совпадения; `401`, `413`, `415`, `422`, `429` — безопасная ошибка доступа, валидации или лимита. |
