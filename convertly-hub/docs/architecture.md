...
- **Telegram linking и recovery:** авторизованный пользователь получает одноразовый deep link через `POST /api/account/telegram/link`. В базе хранится SHA-256-хеш link-токена и его срок жизни; новая pending-ссылка не отменяет уже подтверждённую привязку. Webhook проверяет секретный HTTP-заголовок и принимает команды только из private chat: `/start` и `/help` дают безопасную инструкцию, `/start link_<token>` однократно привязывает chat ID и нормализованный public username, а произвольное сообщение получает нейтральный ответ. `DELETE /api/account/telegram/link` owner-scoped очищает Telegram и незавершённый link token. `POST /api/auth/password-reset/request` принимает email или `@username`, всегда отвечает нейтрально и отправляет reset URL Bot API только в уже подтверждённый активный chat. Username не является доказательством владения.
...
| `POST`, `DELETE /api/account/telegram/link`           | Сессия NextAuth.js                          | `POST` создаёт одноразовую ссылку для привязки Telegram к текущему пользователю; `DELETE` owner-scoped отвязывает Telegram и инвалидирует pending link.                                                                                                                                                                                             | `POST 200` — deep link и срок действия; `DELETE 200` — отвязано; `401` — нет активной сессии; `503` — Telegram не настроен.                                                                                                                                                   |
| `POST /api/telegram/webhook`                          | Заголовок `x-telegram-bot-api-secret-token` | Принимает private-chat обновление Telegram и подтверждает одноразовую привязку по команде `/start link_<token>`; все остальные updates безопасно игнорируются.                                                                                                                                                                                      | `200` — обновление обработано или безопасно проигнорировано; `401` — неверный секрет; `400` — некорректный JSON.                                                                                                                                                              |
| `POST /api/auth/password-reset/request`               | Публичный                                   | Нейтрально принимает email или `@telegram_username`; одноразовая ссылка уходит через SMTP либо только в подтверждённый Telegram chat.                                                                                                                                                                                                               | `202` — одинаковый ответ для существующего и отсутствующего контакта; `400` — невалидный контакт.                                                                                                                                                                             |
...
- **Восстановление пароля:**
  1. Пользователь вводит email или `@username` на `/password-reset`; `POST /api/auth/password-reset/request` всегда отвечает нейтрально и не раскрывает существование учётной записи.
  2. Для существующего пользователя сохраняется только SHA-256-хеш одноразового токена на 30 минут; ссылка доставляется через SMTP (локально — MailHog) или Bot API в подтверждённый Telegram chat. Локальный limiter допускает три запроса на contact в час.
