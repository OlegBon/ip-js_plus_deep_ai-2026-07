...
После первой guest-конвертации сервер также вычисляет месячный `GUEST-…` support
code из visitor cookie, даты начала периода и отдельного server-only
`GUEST_SUPPORT_CODE_SECRET`. В БД записывается только SHA-256-хеш кода. Поэтому
оператор может найти конкретную квоту по коду из обращения в support, но не может
получить исходный cookie пользователя, а публичный HTTP-маршрут не умеет сбрасывать
лимит.
