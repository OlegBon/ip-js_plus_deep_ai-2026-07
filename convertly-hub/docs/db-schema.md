...
Документ описывает Prisma-схему Convertly Hub для PostgreSQL. В репозитории есть миграции `20260824093000_rbac_telegram` для RBAC/Telegram, `20260825090000_subscription_quotas` для тарифов и квот, `20260826120000_quota_reservations` для атомарного резервирования места в хранилище, `20260831110000_conversion_result_reuse` для безопасного повторного использования неистёкшего результата и `20260907170000_telegram_password_recovery` для unique public Telegram username. В новом окружении перед использованием Telegram, тарифов или конвертации примените все миграции командой `npx prisma migrate deploy`.
...
| auth и Telegram-поля     | `String?` / `DateTime?` | Хеши одноразовых токенов, сроки действия и даты подтверждения; `emailVerificationExpires` и `passwordResetExpires` дают reset-ссылкам TTL 30 минут, а `telegramId` и нормализованный `telegramUsername` уникальны. |
