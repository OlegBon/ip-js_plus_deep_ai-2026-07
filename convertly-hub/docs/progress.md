...
- **Изменённые файлы:** `docs/backlog/README.md`, удалённый `docs/backlog/050-telegram-and-account.md`, `docs/work_plan.md`, `docs/audits/documentation-audit-2026-09-07.md`, deployment/layer guides и `docs/progress.md`.
- **Результат:** В production вручную подтверждены привязка Telegram, запрос reset по `@username`, доставка одноразовой ссылки в private chat, смена пароля и повторный вход. Telegram-задача удалена из активного backlog по правилам проекта; решение сохранено в Git, guides и журнале изменений.
...
- **Результат:** Admin Panel поддерживает поиск по email, фильтр статусов, cursor-пагинацию, Refresh с временем последнего успешного обновления, отмену `PENDING`/`FAILED` запросов и audit event `CANCELLED`. Пользователь видит время отправки, может отменить только `PENDING` request; Profile опрашивает состояние раз в 30 секунд. После фактического удаления он автоматически выходит из сессии и перенаправляется на Главную с уведомлением.
- **Проверки:** Prisma validate/generate, TypeScript, ESLint, Jest, Playwright и real integration/E2E выполняются перед merge.
- **Новые переменные окружения:** нет.
...
- **Задача:** Отменить Guest support code и ограниченный reset job.
...
- **Изменённые файлы:** `app/globals.css`, root, auth, dashboard, pricing,
  documentation и `not-found` pages/layouts, `docs/progress.md`.
- **Результат:** применён единый набор Tailwind-классов `py-12 sm:py-16`
  (`48px` / `64px`) для верхнего и нижнего отступов. Короткие страницы остаются центрированными в
  высоте main без лишней прокрутки; главная, Dashboard, Admin, Pricing и Docs
  используют одинаковый вертикальный ритм, когда контент прокручивается.
- **Проверки:** TypeScript, ESLint и Playwright critical flows выполняются перед
  финальным коммитом; desktop/mobile visual QA выполнен локально.
- **Новые переменные окружения:** не добавлялись.

# 2026-09-03

- **Задача:** Исправить `InvalidKey` при сохранении авторизованной конвертации
  в Supabase Storage.
- **Изменённые файлы:** `lib/privacy/conversion-results.ts`,
  `lib/privacy/__tests__/conversion-results.test.ts`, `docs/progress.md`.
- **Результат:** S3 object key больше не содержит пользовательское имя файла и
  URI-кодированные символы. Новый ключ имеет стабильную форму
  `users/<userId>/conversions/<conversionId>/result.<extension>`; скачиваемое
  имя продолжает читаться из `ConversionLog.resultFileName`. Ранее сохранённые
  результаты остаются доступны, поскольку их прежние ключи уже хранятся в БД.
...
- **Описание:** В футер добавлена доступная ссылка `mailto:support@bon.kharkov.ua`.
  ...
  на desktop они стоят в строке, на mobile переносятся целыми смысловыми блоками.
- **Изменённые файлы:** `components/core/Footer.tsx`, его component-тест и
  `docs/progress.md`.
- **Проверки:** целевой Jest-тест, TypeScript, ESLint и desktop/mobile Playwright
