...
- **Описание:** История Dashboard загружается страницами по 10 строк через `GET /api/account/conversions`; кнопки Previous/Next используют cursor следующей страницы, без `OFFSET` и без загрузки всего журнала в браузер.
