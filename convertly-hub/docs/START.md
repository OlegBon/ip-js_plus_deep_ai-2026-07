...
> **Статус на 24 августа 2026:** Docker Compose, Prisma, NextAuth, RBAC, Telegram linking и API приёма конвертаций проверены локально. `POST /api/v1/convert` пока создаёт задание `PENDING`; фактическая обработка файлов будет подключена на этапе Core.
